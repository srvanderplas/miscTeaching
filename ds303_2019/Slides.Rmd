...
```{r, eval = F}
library(tidyverse)
install.packages(c("parsnip", "broom", "yardstick", "rsample", "tidymodels", "AmesHousing"))
```
...
models <- tibble::tibble(model_name = c("random_forest", "linear_regression"), 
                         model_fcn = list(rf_model, linear_model)) %>%
  dplyr::mutate(
    res = purrr::map(
      model_fcn, ~fit(., formula = log10(Sale_Price) ~ Longitude + Latitude + 
                        Lot_Area + Neighborhood + Year_Sold, data = ames_train)),
    pred = purrr::map(res, predict, new_data = ames_test)
  )
```
...
ames_cv <- vfold_cv(ames_train_1var, v = 100, strata = "Sale_Price", breaks = 8)
ames_cv[1,]
ames_cv$splits[[1]]
...
ames_lm <- function(cv) lm(formula = log10(Sale_Price) ~ Gr_Liv_Area,
...
output_range <- tibble(Gr_Liv_Area = min(ames$Gr_Liv_Area):max(ames$Gr_Liv_Area)) 

# Get regression lines and assessment set predictions
ames_cv_loess <- ames_cv %>% 
  mutate(
    model = purrr::map(splits, ames_loess),
    pred_line = purrr::map(model, predict, newdata = output_range),
    pred_line = purrr::map(
      pred_line, ~bind_cols(output_range, pred = .)),
    pred_assessment = purrr::map2(
      model, splits, ~predict(.x, newdata = assessment(.y))),
    pred_assessment = purrr::map2(
      pred_assessment, splits, ~bind_cols(assessment(.y), pred = .x)))
```
...
```{r}
select(ames_cv_loess, id, pred_line) %>%
  unnest() %>%
  ggplot(aes(x = Gr_Liv_Area, y =  pred, group = id)) + 
  geom_line(alpha = .05)
```
